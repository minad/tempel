#+title: tempel.el - News
#+author: Daniel Mendler
#+language: en

* Version 1.14 (2026-07-05)

- =tempel--templates=: Return templates in order of hooks.
...
- =tempel-done=, =tempel-abort=: Add =ALL= prefix argument to finish or abort all
  templates. If =ALL= is nil, treat only the template at point.
- =tempel-done-on-region=, =tempel-done-on-next=: New options to customize quitting
  behavior. If =tempel-done-or-region= is non-nil, quit when entering a region
  field. If =tempel-done-on-next= is non-nil, quit when pressing =tempel-next= from
  inside the last field.

* Version 1.8 (2025-11-15)

- =tempel-trigger-prefix=: Remove feature. Use =cape-capf-trigger= from the Cape
  package instead. See also =corfu-auto-trigger=.
