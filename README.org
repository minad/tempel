...
The package is available from MELPA and can be installed with =package-install=.
The following example configuration relies on =use-package=. For some ready-made
templates check out the package [[https://github.com/Crandel/tempel-collection][tempel-collection]]. We appreciate if more
templates are contributed there.
...
~tempel-path~, and the variables ~tempel-global-templates~ and
~tempel-local-templates~.

#+begin_src emacs-lisp
(setq tempel-global-templates
...
(add-hook
 ...
 (lambda () (setq tempel-local-templates my-elisp-templates)))
#+end_src
