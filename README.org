...
data files are files containing Lisp s-expressions (see ~lisp-data-mode~). By default the file
"templates" in the `user-emacs-directory' is used. The templates are grouped by
